...
try {
    ...
    Phroses::setMaintenance(Phroses::MM_ON);
    ...
    $out->send("progress", [ "progress" => 70 ]);
    
    // finish update
    @shell_exec("php phroses.phar update");
    $out->send("progress", [ "progress" => 100, "version" => $version ]);
    
} catch(PharException $e) {
    $out->send("error", ["error" => "extract"]);

} catch(WriteException $e) {
    $out->send("error", ["error" => "write", "action" => $e->action, "file" => $e->file ]);
    
} catch(Exception $e) {
    $out->send("error", ["error" => $e->getMessage() ]);     
    ...
    Phroses::setMaintenance(Phroses::MM_OFF);
}
