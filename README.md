...
Phroses is a multi-site content management system and dispatcher.  Phroses is safe to use in production, with basic page creation, editing and theming features implemented.  Many advanced / extra features & systems are still being implemented and ironed out. Phroses is easy to use, and extensible with customizable themes using [phyrex](https://github.com/cythral/phyrex) templating, along with plugins using [listen](https://github.com/cythral/listen).  Phroses has only been tested on apache, support for nginx is coming later on. 
